...
def main(project_name):
    ...
    if not (os.path.exists(PROJECT_PATH + "\\analysis\\func to commits message.txt")):
        print("missing data")

    else:
        if not (os.path.exists(PROJECT_PATH + "\\topicModeling")):
            os.mkdir(PROJECT_PATH + "\\topicModeling")

        if not (os.path.exists(PROJECT_PATH + "\\topicModeling\\bug to funcion and similarity")):
            os.mkdir(PROJECT_PATH + "\\topicModeling\\bug to funcion and similarity")

        if not (os.path.exists(PROJECT_PATH + "\\topicModeling\\topics")):
            os.mkdir(PROJECT_PATH + "\\topicModeling\\topics")

        if not (os.path.exists(PROJECT_PATH + "\\topicModeling\\filtered_data.txt")):
            with open(PROJECT_PATH + "\\analysis\\func to commits message.txt") as outfile:
    ...
    # data cleaning
            ...
            save_into_file("word_to_counts", word_to_counts, 'words')
            save_into_file("filtered_data" , func_to_prepared_commit_messages, 'strings')
            save_into_file("function_to_avg_commit_len", func_to_avg, 'function to avg')
        else:
            with open(PROJECT_PATH + "\\topicModeling\\filtered_data.txt") as outfile:
                func_to_prepared_commit_messages = json.load(outfile)['strings']
            with open(PROJECT_PATH + "\\topicModeling\\word_to_counts.txt") as outfile:
        ...
        pickle.dump(corpus, open(PROJECT_PATH +'\\topicModeling\\corpus.pkl', 'wb'))
        dictionary.save(PROJECT_PATH +'\\topicModeling\\dictionary.gensim')
        ...
        for NUM_TOPICS in range(15,26):
            ...
            num_topics_to_table.extend(bug_to_func_and_similarity(ldamodel, dictionary,func_to_prepared_commit_messages,NUM_TOPICS))
            ldamodel.save(PROJECT_PATH +'\\topicModeling\\topics\\model'+ str(NUM_TOPICS)+'.gensim')
        ...
        create_table(num_topics_to_table)




def bug_to_func_and_similarity( lda,dictionary,prepared_commit_messages, NUM_TOPICS):
    with open(PROJECT_PATH + "\\analysis\\bug_to_commit_that_solved.txt") as outfile:
            bugs = json.load(outfile)['bugs to commit']

    if not (os.path.exists(PROJECT_PATH + "\\topicModeling\\bug to funcion and similarity\\bug to functions and similarity " + str(NUM_TOPICS) + " topics.txt")):
        all_bugs = []  # del
        ...
        save_into_file("bug to funcion and similarity\\bug to functions and similarity " + str(NUM_TOPICS) + " topics" ,bug_to_func_and_similarity,'bugs')


    with open(PROJECT_PATH + "\\topicModeling\\bug to funcion and similarity\\bug to functions and similarity " + str(NUM_TOPICS) + " topics.txt") as outfile:
    ...
    return fill_table(NUM_TOPICS,bugs,bug_to_func_and_similarity, commit_to_exist_functions)



def fill_table(NUM_TOPICS, bugs , bug_to_func_and_similarity, commit_to_exist_functions):
    ret_list = []  # will hold tuples that each one represent bug id, num of funcs that changed, max index of changed func
    # num_of_indexses_above_2000 = 0
    # num_of_indexses_above_2000_no_test = 0
    # sum_index = 0
    # sum_index_no_tests = 0
    i= 1
    for bug in bugs:
        if len(bug['function that changed']) > 10:
            continue
        # index = find_max_index(bug,bug_to_func_and_similarity ,commit_to_exist_functions[str(bug['commit number'])])
        # if index[0] > 2000:
        #     num_of_indexses_above_2000 += 1
        # if index[1] > 2000:
            #num_of_indexses_above_2000_no_test += 1

        index_len_all_funcs = find_max_index_all_functions(bug,bug_to_func_and_similarity)
        index_len_all_funcs_no_tests = find_max_index_all_functions_no_tests(bug,bug_to_func_and_similarity)
        index_len_exist_funcs = find_max_index_exist_functions(bug,bug_to_func_and_similarity, commit_to_exist_functions[str(bug['commit number'])]['all functions'])
        index_len_exist_funcs_no_tests = find_max_index_exist_functions_no_tests(bug,bug_to_func_and_similarity, commit_to_exist_functions[str(bug['commit number'])]['all functions'])
        ...
        # sum_index += index[0]
        # sum_index_no_tests += index[1]

        print("finished bug number " + str(i))
        i += 1

    # num_of_bugs = len(list)

    # list.append(["above ", "index ", "2000","->",str(num_of_indexses_above_2000)])
    # list.append(["above ", "index ", "2000","without tests->",str(num_of_indexses_above_2000_no_test)])
    # list.append(["avg ", "index ", "with tests","->",str(sum_index/num_of_bugs)])
    # list.append(["avg ", "index ", "without tests","->",str(sum_index_no_tests/num_of_bugs)])

    return ret_list


def find_max_index_all_functions(bug,bug_to_func_and_similarity):
    max_index = -1
...
def find_max_index_all_functions_no_tests(bug,bug_to_func_and_similarity):
...
def find_max_index_exist_functions(bug,bug_to_func_and_similarity ,exists_functions):
...
def find_max_index_exist_functions_no_tests(bug,bug_to_func_and_similarity ,exists_functions):
...
def create_table(rows):
    with open(PROJECT_PATH +'\\topicModeling\\table.csv', 'w', newline='', ) as file:
...
def save_into_file(file_name, new_data, dictionary_value):
    ...
    with open(PROJECT_PATH + "\\topicModeling\\" + file_name + ".txt", 'w') as outfile:
        json.dump(data, outfile, indent=4)


def visualize(NUM_TOPICS):

    dictionary = gensim.corpora.Dictionary.load(PROJECT_PATH +'\\topicModeling\\dictionary.gensim')
    corpus = pickle.load(open(PROJECT_PATH +'\\topicModeling\\corpus.pkl', 'rb'))
    lda = gensim.models.ldamodel.LdaModel.load(PROJECT_PATH +'\\topicModeling\\topics\\model'+ str(NUM_TOPICS)+'.gensim')
    lda_display = pyLDAvis.gensim.prepare(lda, corpus, dictionary, sort_topics=False)
    pyLDAvis.display(lda_display)

if __name__ == "__main__":
    main("apache_commons-lang")
